...
public class KafkaStreamsApplication {
	...
	static Topology buildTopology(String inputTopic, String outputTopic) {
		...
		builder.stream(inputTopic, Consumed.with(stringSerde, stringSerde))
				...
				.to(outputTopic, Produced.with(stringSerde, stringSerde, new CustomStreamPartitioner()));
	...
	public static void main(String[] args) throws Exception {
		...
		try (Util utility = new Util()) {
			...
			utility.createTopics(props, Arrays.asList(
					...
					new NewTopic(outputTopic, Optional.of(CustomStreamPartitioner.alphabetSize), Optional.empty())));
